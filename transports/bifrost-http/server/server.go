...
import (
	...
	dynamicPlugins "github.com/Gaurav-Gosain/bifrost/framework/plugins"
	"github.com/Gaurav-Gosain/bifrost/framework/tracing"
	"github.com/Gaurav-Gosain/bifrost/plugins/governance"
	"github.com/Gaurav-Gosain/bifrost/plugins/logging"
	"github.com/Gaurav-Gosain/bifrost/plugins/semanticcache"
...
// PrefixRoute adds the base path prefix to a route
func (s *BifrostHTTPServer) PrefixRoute(route string) string {
	if s.BasePath == "" {
		return route
	}
...
// GetBasePath returns the normalized base path
func (s *BifrostHTTPServer) GetBasePath() string {
	return s.BasePath
}
...
func (s *BifrostHTTPServer) RegisterAPIRoutes(ctx context.Context, callbacks ServerCallbacks, middlewares ...schemas.BifrostHTTPMiddleware) error {
	...
	if err == nil && prometheusPlugin.GetRegistry() != nil {
		...
		s.Router.GET(handlers.PrefixRoute("/metrics"), metricsHandler)
	} else {
...
func (s *BifrostHTTPServer) Bootstrap(ctx context.Context) error {
	...
	// Set base path for all handlers before route registration
	s.BasePath = NormalizeBasePath(s.BasePath)
	handlers.SetBasePath(s.BasePath)
