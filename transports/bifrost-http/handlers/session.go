...
func (h *SessionHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.POST(PrefixRoute("/api/session/login"), lib.ChainMiddlewares(h.login, middlewares...))
	r.POST(PrefixRoute("/api/session/logout"), lib.ChainMiddlewares(h.logout, middlewares...))
	r.GET(PrefixRoute("/api/session/is-auth-enabled"), lib.ChainMiddlewares(h.isAuthEnabled, middlewares...))
}
