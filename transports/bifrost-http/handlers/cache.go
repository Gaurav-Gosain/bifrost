...
func (h *CacheHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.DELETE(PrefixRoute("/api/cache/clear/{requestId}"), lib.ChainMiddlewares(h.clearCache, middlewares...))
	r.DELETE(PrefixRoute("/api/cache/clear-by-key/{cacheKey}"), lib.ChainMiddlewares(h.clearCacheByKey, middlewares...))
}
