...
func (h *DevPprofHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.GET(PrefixRoute("/api/dev/pprof"), lib.ChainMiddlewares(h.getPprof, middlewares...))
	r.GET(PrefixRoute("/api/dev/pprof/goroutines"), lib.ChainMiddlewares(h.getGoroutines, middlewares...))
}
