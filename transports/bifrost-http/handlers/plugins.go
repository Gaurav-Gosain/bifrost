...
import (
	...
	"github.com/Gaurav-Gosain/bifrost/framework/plugins"
...
func (h *PluginsHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.GET(PrefixRoute("/api/plugins"), lib.ChainMiddlewares(h.getPlugins, middlewares...))
	r.GET(PrefixRoute("/api/plugins/{name}"), lib.ChainMiddlewares(h.getPlugin, middlewares...))
	r.POST(PrefixRoute("/api/plugins"), lib.ChainMiddlewares(h.createPlugin, middlewares...))
	r.PUT(PrefixRoute("/api/plugins/{name}"), lib.ChainMiddlewares(h.updatePlugin, middlewares...))
	r.DELETE(PrefixRoute("/api/plugins/{name}"), lib.ChainMiddlewares(h.deletePlugin, middlewares...))
}
