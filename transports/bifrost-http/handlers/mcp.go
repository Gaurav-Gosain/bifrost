...
func (h *MCPHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.GET(PrefixRoute("/api/mcp/clients"), lib.ChainMiddlewares(h.getMCPClients, middlewares...))
	r.POST(PrefixRoute("/api/mcp/client"), lib.ChainMiddlewares(h.addMCPClient, middlewares...))
	r.PUT(PrefixRoute("/api/mcp/client/{id}"), lib.ChainMiddlewares(h.updateMCPClient, middlewares...))
	r.DELETE(PrefixRoute("/api/mcp/client/{id}"), lib.ChainMiddlewares(h.deleteMCPClient, middlewares...))
	r.POST(PrefixRoute("/api/mcp/client/{id}/reconnect"), lib.ChainMiddlewares(h.reconnectMCPClient, middlewares...))
	r.POST(PrefixRoute("/api/mcp/client/{id}/complete-oauth"), lib.ChainMiddlewares(h.completeMCPClientOAuth, middlewares...))
}
