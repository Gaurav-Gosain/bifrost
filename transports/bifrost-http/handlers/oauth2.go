...
func (h *OAuthHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.GET(PrefixRoute("/api/oauth/callback"), lib.ChainMiddlewares(h.handleOAuthCallback, middlewares...))
	r.GET(PrefixRoute("/api/oauth/config/{id}/status"), lib.ChainMiddlewares(h.getOAuthConfigStatus, middlewares...))
	r.DELETE(PrefixRoute("/api/oauth/config/{id}"), lib.ChainMiddlewares(h.revokeOAuthConfig, middlewares...))
}
